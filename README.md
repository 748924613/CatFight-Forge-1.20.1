...
- 猫饼会以方头、长身、四条横腿和细尾巴的专用**一像素厚**扁平轮廓显示；猫饼状态使用干净、全不透明的浅色专用贴图，不再把原版猫皮反复拉伸而产生花纹或透明缺块。它会完全固定在撞击位置、不能移动或转向，也不会参与对峙。用**空玻璃瓶**右键猫饼即可救回（玻璃瓶不会消耗）；十分钟无人救援会直接死亡。
- 新增 3 个物品：
...
2. 将构建产物 `catfight-1.3.19-forge-1.20.1.jar` 放进 `.minecraft/mods`。
